...
(() => {
  ...
  const FAV_KEY = 'motors-events-favorites';
  const getFavs = () => JSON.parse(localStorage.getItem(FAV_KEY) || '[]');
  const setFavs = v => { localStorage.setItem(FAV_KEY, JSON.stringify(v)); updateFavCount(); };
  ...
  function openEvent(id){
    ...
    left.append(el('div',{class:'detail-box'},
      ...
      el('div',{class:'detail-actions'},
        ...
        el('a',{class:'btn btn-orange',href:e.url,target:'_blank',rel:'noopener',text:'Site officiel'})
      )
    ));
  ...
  $('#alertCat').insertAdjacentHTML('afterbegin','<option value="all">Toutes les catégories</option>');
  ...
  $('#alertsForm').addEventListener('submit',ev=>{ev.preventDefault();localStorage.setItem('motors-events-alert',JSON.stringify({cat:$('#alertCat').value,region:$('#alertRegion').value,keyword:$('#alertKeyword').value}));$('#alertStatus').textContent='Alerte enregistrée sur cet appareil.';});
